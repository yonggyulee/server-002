...
public class UpdateClassificationGtDatasetHandler : GtDatasetHandler, IRequestHandler<UpdateClassificationGtDatasetCommand, ClassificationGtDataset>
{
    private readonly RequesterContext _requesterContext;

    public UpdateClassificationGtDatasetHandler(IDbContextFactory<DatasetDbContextPostgreSQL> dbContextFactory,
        IPostgresLockProviderFactory lockProviderFactory, ILogger<UpdateClassificationGtDatasetHandler> logger,
    ...
    public async Task<ClassificationGtDataset> Handle(UpdateClassificationGtDatasetCommand command, CancellationToken cancellationToken)
    {
        ...
        var selectedDataset = await _dbContext.ClassificationGtDatasets
                                  ...
                                  .SingleOrDefaultAsync(d => d.Id == request.Id,
                                      cancellationToken)
                              ?? throw new InvalidOperationException($"Classification Id={request.Id}");
